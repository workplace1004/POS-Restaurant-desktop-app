...
let s = fs.readFileSync(controlPath, 'utf8');
const startMarker = "          ) : topNavId === 'cash-register' ? (";
...
const next = s.slice(0, start) + replacement + s.slice(end + endMarker.length);
fs.writeFileSync(controlPath, next);
console.log('Patched ControlView main area, removed', end + endMarker.length - start, 'chars');
