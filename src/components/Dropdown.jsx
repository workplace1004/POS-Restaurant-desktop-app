 ...
 * @param {string} [props.className] - Additional classes for the outer wrapper (layout, min-width)
 * @param {string} [props.labelClassName] - Additional classes for the option list container
 * @param {boolean} [props.hasError] - When true, trigger shows validation styling (rose border/background) on the visible control
 */
export function Dropdown({ options = [], value, onChange, placeholder = 'Select…', disabled = false, inline = false, className = '', labelClassName = '', hasError = false }) {
  ...
  const listCommonClasses = `w-full py-1 bg-pos-bg border border-gray-300 rounded-lg shadow-lg max-h-[50px] overflow-auto z-[10000] [scrollbar-width:thin] [scrollbar-color:#9ca3af_#34495e] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-pos-panel [&::-webkit-scrollbar-thumb]:bg-gray-400 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:border [&::-webkit-scrollbar-thumb]:border-pos-panel active:[&::-webkit-scrollbar-thumb]:bg-gray-300 ${labelClassName}`;

  const listContent = open ? (
    <ul
      ref={listRef}
      className={inline ? `absolute top-full left-0 ${listCommonClasses}` : `fixed ${listCommonClasses}`}
      role="listbox"
    ...
    >
      {options.map((opt) => (
        <li
          ...
          className={`px-4 py-2 cursor-pointer text-white text-sm transition-colors ${opt.value === value ? 'bg-pos-panel font-medium' : 'text-gray-800 active:bg-green-500'}`}
          onClick={() => {
            onChange(opt.value);
            setOpen(false);
          }}
  ...
  const triggerBase =
    'w-full flex items-center h-[40px] justify-between gap-2 px-4 py-3 text-left rounded-lg text-white text-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none';
  const triggerNormal =
    'border border-gray-300 bg-pos-panel text-gray-800 active:border-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent';
  const triggerError =
    'border-2 border-rose-500 bg-rose-500/40 text-gray-100 active:border-rose-500 focus:ring-2 focus:ring-rose-500/70 focus:border-rose-500';
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        ...
        className={`${triggerBase} ${hasError ? triggerError : triggerNormal}`}
        aria-haspopup="listbox"
        aria-expanded={open}
        aria-label={displayLabel}
      >
        <span className="min-w-0 truncate">{displayLabel}</span>
        <svg className={`w-5 h-5 shrink-0 transition-transform ${hasError ? 'text-rose-100' : 'text-gray-500'} ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
