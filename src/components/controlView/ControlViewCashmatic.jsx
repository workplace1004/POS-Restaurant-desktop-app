...
export function ControlViewCashmatic({
  ...
  return (
    <div className="relative pb-8 pt-4">
      <div className="grid grid-cols-1 mt-6 text-lg md:grid-cols-2 gap-x-10 gap-y-4 mb-6">
        <div className="flex items-center gap-2">
          <label className="block text-black font-medium min-w-[120px] max-w-[120px] shrink-0">{tr('control.cashmatic.name', 'Name *')}</label>
          <input
            type="text"
            value={cashmaticName}
            ...
            className="px-4 w-[200px] py-3 bg-pos-panel h-[40px] border border-gray-300 rounded-lg text-gray-200 placeholder-pos-muted focus:outline-none focus:border-rose-500"
          />
        </div>
        <div></div>
        <div className="flex items-center gap-2">
          <label className="block text-black font-medium min-w-[120px] max-w-[120px] shrink-0">{tr('control.cashmatic.connectionType', 'Connection type *')}</label>
          <div className="flex gap-2">
            <button
              type="button"
              className={`px-6 py-2 text-lg font-medium rounded-lg ${cashmaticConnectionType === 'tcp' ? 'bg-rose-600 text-white' : 'text-pos-text border border-gray-300 text-black'} active:bg-rose-500`}
            ...
            <button
              type="button"
              className={`px-6 py-2 text-lg font-medium rounded-lg ${cashmaticConnectionType === 'api' ? 'bg-rose-600 text-white' : 'text-pos-text border border-gray-300 text-black'} active:bg-rose-500`}
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="px-4 w-[200px] py-3 bg-pos-panel h-[40px] border border-gray-300 rounded-lg text-gray-200 focus:outline-none focus:border-rose-500"
              />
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="flex justify-center pt-5 pb-5">
        <button type="button" className="flex items-center text-lg gap-4 px-[70px] py-3 rounded-lg bg-rose-600 text-white font-medium active:bg-rose-500 disabled:opacity-50" disabled={savingCashmatic} onClick={handleSaveCashmatic}>
      ...
      <div className="absolute shrink-0 w-full flex justify-center">
