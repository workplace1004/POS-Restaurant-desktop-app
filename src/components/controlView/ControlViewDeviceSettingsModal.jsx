...
/** These tabs stay visible but cannot be opened from this UI. */
...
export function ControlViewDeviceSettingsModal(props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative text-lg bg-pos-bg rounded-xl shadow-2xl max-w-[1430px] h-[800px] w-full mx-4 overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        <button type="button" className="absolute top-4 right-4 z-10 p-2 rounded text-black active:text-white active:bg-rose-500" onClick={closeDeviceSettingsModal} aria-label="Close">
          <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
        <div className="flex mt-16 mb-4 px-6 w-full justify-around  shrink-0 overflow-x-auto">
          {DEVICE_SETTINGS_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                type="button"
                disabled={disabled}
                className={`px-4 py-2 font-medium whitespace-nowrap border-b-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:pointer-events-none ${deviceSettingsTab === tab ? 'border-rose-500 text-black' : 'border-transparent text-black'} ${!disabled ? 'active:text-white active:bg-rose-500' : ''}`}
        ...
        <div className="p-4 overflow-auto flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4">
              <div className="flex flex-col gap-4">
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <input type="checkbox" checked={deviceAutoReturnToTablePlan} onChange={(e) => setDeviceAutoReturnToTablePlan(e.target.checked)} className="w-9 h-9 rounded border-gray-300" />
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Dropdown options={mapTranslatedOptions(vatOptions)} value={deviceCounterSaleVat} onChange={setDeviceCounterSaleVat} placeholder={tr('control.external.select', 'Select')} className=" min-w-[150px]" />
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-pos-text min-w-[270px] max-w-[270px] shrink-0">{tr('control.device.general.tableSaleVat', 'Table sale VAT:')}</span>
                  <Dropdown options={mapTranslatedOptions(vatOptions)} value={deviceTableSaleVat} onChange={setDeviceTableSaleVat} placeholder={tr('control.external.select', 'Select')} className=" min-w-[150px]" />
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" className="p-2 px-3 rounded border border-pos-border text-black active:bg-rose-500 active:text-white font-medium" onClick={() => setDeviceTimeoutLogout((n) => Math.max(0, n - 1))}>−</button>
                    <input type="number" min={0} value={safeNumberInputValue(deviceTimeoutLogout, 0)} onChange={(e) => setDeviceTimeoutLogout(Number(e.target.value) || 0)} className="w-16 px-2 py-2 border border-gray-300 rounded text-pos-text text-center h-[45px]" />
                    <button type="button" className="p-2 px-3 rounded border border-pos-border text-black active:bg-rose-500 active:text-white font-medium" onClick={() => setDeviceTimeoutLogout((n) => n + 1)}>+</button>
                  </div>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Dropdown options={[{ value: '-', label: '-' }]} value="-" onChange={() => { }} placeholder="-" className=" min-w-[120px] opacity-60 pointer-events-none" disabled />
                </div>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4">
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Dropdown options={mapTranslatedOptions(printingOrderOptions)} value={devicePrinterNextCourseOrder} onChange={setDevicePrinterNextCourseOrder} placeholder={tr('control.external.asRegistered', 'As Registered')} className=" min-w-[200px] max-w-[200px]" />
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <Dropdown options={mapTranslatedOptions(groupingReceiptOptions)} value={devicePrinterStandardMode} onChange={setDevicePrinterStandardMode} placeholder={tr('control.external.enable', 'Enable')} className=" min-w-[200px] max-w-[200px]" />
                </div>
                <div className="flex items-center gap-3">
          ...
          {deviceSettingsTab === 'Category display' && (
            <div className="grid grid-cols-1 px-4 md:grid-cols-3 lg:grid-cols-3 gap-4">
              {categories.map((cat) => {
                ...
                return (
                  <label key={cat.id} className="flex items-center gap-5 cursor-pointer">
                    ...
                    />
                    <span className="text-pos-text  min-w-[250px] max-w-[250px] truncate">{cat.name || cat.id}</span>
                  </label>
                );
              })}
            </div>
          ...
          {deviceSettingsTab === 'Orders in waiting' && (
            <div className="grid px-4 grid-cols-1 md:grid-cols-2 gap-y-4">
              <div className="flex flex-col gap-4">
                <label className="flex items-center gap-3 cursor-pointer">
          ...
          {deviceSettingsTab === 'Scheduled orders' && (
            <div className="grid grid-cols-1 md:grid-cols-2 px-4 gap-y-4">
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-5">
                  ...
                  <Dropdown options={mapTranslatedOptions(SCHEDULED_ORDERS_PRODUCTION_FLOW_OPTIONS)} value={deviceScheduledProductionFlow} onChange={setDeviceScheduledProductionFlow} placeholder={tr('control.external.select', 'Select')} className=" min-w-[150px] max-w-[150px]" />
                </div>
                <div className="flex items-center gap-5">
                  ...
                  <Dropdown options={mapTranslatedOptions(SCHEDULED_ORDERS_LOADING_OPTIONS)} value={deviceScheduledLoading} onChange={setDeviceScheduledLoading} placeholder={tr('control.external.select', 'Select')} className=" min-w-[150px] max-w-[150px]" />
                </div>
                <div className="flex items-center gap-5">
                  <span className="text-pos-text min-w-[270px] max-w-[270px] shrink-0">{tr('control.device.scheduled.mode', 'Scheduled order mode:')}</span>
                  <Dropdown options={mapTranslatedOptions(SCHEDULED_ORDERS_MODE_OPTIONS)} value={deviceScheduledMode} onChange={setDeviceScheduledMode} placeholder={tr('control.external.select', 'Select')} className=" min-w-[150px] max-w-[150px]" />
                </div>
                <div className="flex items-center gap-5">
                  ...
                  <Dropdown options={mapTranslatedOptions(SCHEDULED_ORDERS_INVOICE_LAYOUT_OPTIONS)} value={deviceScheduledInvoiceLayout} onChange={setDeviceScheduledInvoiceLayout} placeholder={tr('control.external.select', 'Select')} className=" min-w-[150px] max-w-[150px]" />
                </div>
                <div className="flex items-center gap-5">
                  ...
                  <Dropdown options={mapTranslatedOptions(SCHEDULED_ORDERS_CHECKOUT_AT_OPTIONS)} value={deviceScheduledCheckoutAt} onChange={setDeviceScheduledCheckoutAt} placeholder={tr('control.external.select', 'Select')} className=" min-w-[150px] max-w-[150px]" />
                </div>
              </div>
              <div className="flex flex-col gap-4">
                <label className="flex items-center gap-3 cursor-pointer">
        ...
        <div className="w-full flex items-center px-4 pt-5 pb-5 justify-center shrink-0">
          <button
            type="button"
            className="flex items-center text-xl gap-4 px-[70px] py-3 rounded-lg bg-rose-600 text-white font-medium active:bg-rose-500 disabled:opacity-50"
