  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative bg-pos-bg rounded-xl shadow-2xl max-w-[90%] w-full justify-center items-center mx-4 overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        <button type="button" className="absolute top-4 right-4 z-10 p-2 rounded text-black active:text-white active:bg-rose-500" onClick={closePriceGroupModal} aria-label="Close">
        ...
        <div className="p-6 flex flex-col space-y-6 w-full justify-center items-center pt-20">
          <div className='w-full flex flex-col justify-center items-center gap-10'>
            <div className="flex gap-2 w-full items-center justify-center">
              <label className="block text-lg min-w-[100px] font-medium text-black">{tr('name', 'Name')} : </label>
              <input
                type="text"
                value={priceGroupName}
                onChange={(e) => setPriceGroupName(e.target.value)}
                placeholder={tr('control.enterName', 'Enter name')}
                className="px-4 w-[200px] h-[40px] py-3 text-lg border border-gray-300 rounded-lg text-black focus:outline-none focus:border-rose-500"
              />
            </div>
            <div className="flex gap-2 w-full items-center justify-center">
              <label className="block text-lg pr-[60px] font-medium text-black">{tr('control.vat', 'VAT')} : </label>
              <Dropdown
        ...
        <div className="flex justify-center py-10">
          <button
            type="button"
            className="flex items-center text-xl gap-4 px-[70px] py-3 rounded-lg bg-rose-600 text-white font-medium active:text-white active:bg-green-500 disabled:opacity-50"
