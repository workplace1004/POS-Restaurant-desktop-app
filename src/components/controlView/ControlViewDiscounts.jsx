    ...
    <div className="relative min-h-[1800px] rounded-xl border border-pos-border bg-pos-panel/30 p-4 pb-[80px]">
      ...
      {discounts.length === 0 ? (
        <ul className="w-full flex flex-col">
          <li className="text-pos-muted text-xl font-medium text-center py-4">{tr('control.discounts.empty', 'No discounts yet.')}</li>
        </ul>
      ) : (
        <>
          <div
            ref={discountsListRef}
            className="max-h-[1640px] min-h-[1640px] bg-white overflow-y-auto rounded-lg [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
          ...
          >
            <ul className="w-full flex flex-col">
              {discounts.map((d) => (
                ...
                >
                  <span className="font-medium truncate min-w-0 flex-1" title={d.name || '—'}>{d.name || '—'}</span>
