            ...
            <ul className="w-full flex flex-col">
              {kitchens.map((m) => {
                ...
                return (
                  ...
                  >
                    <span className="min-w-0 flex-1 font-medium truncate" title={m.name || '—'}>
                      {m.name || '—'}
                    </span>
                    <div className="justify-center min-w-0 absolute left-1/2">
                      <button
                        type="button"
                        className={`shrink-0 py-1.5 text-xs font-semibold active:bg-green-500 active:border-white/30 ${
                          kitchenHasProducts ? 'text-white' : 'text-pos-muted'
                        }`}
