  ...
  return (
    <div className="relative min-h-[650px] rounded-xl border border-pos-border bg-pos-panel/30 p-4 pb-[60px] flex flex-col">
            ...
            {subproductGroups.map((grp) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium whitespace-nowrap shrink-0 transition-colors border-b-2 ${
                  selectedSubproductGroupId === grp.id
      ...
      >
        ...
        ) : subproductGroupsLoading || subproductsLoading ? null : subproducts.length === 0 ? (
            ...
            {subproducts.map((sp) => (
              <li
                ...
                className={`flex items-center w-full justify-between px-4 py-2 border-y border-pos-panel text-pos-text text-sm cursor-pointer ${
                  selectedSubproductId === sp.id ? 'bg-pos-panel/70' : 'bg-pos-bg'
                }`}
                onClick={(e) => {
                  if (!e.target.closest('button')) setSelectedSubproductId(sp.id);
                }}
              >
                <span className="flex-1 font-medium truncate" title={sp.name}>
                  {sp.name}
                </span>
