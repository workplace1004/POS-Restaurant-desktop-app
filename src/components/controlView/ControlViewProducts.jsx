...
export function ControlViewProducts({
  ...
  return (
    <div className="relative min-h-[1800px] rounded-xl border border-pos-border bg-pos-panel/30 p-4 pb-[80px] flex flex-col">
      {/* Action bar: New Product, Search */}
      <div className="flex items-center w-full justify-center gap-4 mb-2 flex-wrap">
        <button
          type="button"
          disabled={!selectedCategoryId}
          onClick={openProductModal}
          className="px-6 py-3 rounded-lg text-lg font-medium border border-pos-border active:text-white text-black active:bg-rose-500 active:border-white/30 transition-colors disabled:opacity-50"
        >
        ...
        <input
          ...
          className="px-4 py-3 rounded-lg bg-pos-bg border border-pos-border z-[20] text-black text-lg min-w-[200px] placeholder:text-pos-muted cursor-pointer"
        />
      </div>
          ...
          >
            {categories.map((cat) => (
              <button
                ...
                type="button"
                className={`px-4 py-2 text-lg font-medium whitespace-nowrap shrink-0 transition-colors border-b-2 ${selectedCategoryId === cat.id
                  ? 'text-pos-text border-rose-500'
                  : 'text-pos-muted active:text-pos-text bg-transparent border-transparent active:bg-rose-500'
                  }`}
      ...
      <div
        ref={productsListRef}
        className="flex-1 overflow-auto max-h-[1600px] min-h-0 bg-pos-bg [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
          ...
          <ul className="w-full flex flex-col">
            {filteredProducts.map((product) => {
              ...
              return (
                <li
                  key={product.id}
                  className={`flex items-center w-full justify-between px-4 py-2  text-pos-text text-lg cursor-pointer ${selectedProductId === product.id ? 'bg-pos-panel/70' : 'bg-pos-bg'}`}
                ...
                >
                  ...
                  <span className="absolute right-1/3 flex flex-shrink-0 min-w-[30%] items-center justify-center gap-3 text-center text-lg">
                    <button
                      type="button"
                      className={`px-2 py-1 rounded text-lg active:text-white active:bg-rose-500 ${hasSubproducts ? 'text-black font-medium' : 'text-gray-500/50 active:text-white'}`}
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 rounded px-2 py-1 text-lg active:text-white active:bg-rose-500 ${hasSubproducts ? 'text-black font-medium' : 'text-pos-muted'}`}
                      onClick={(e) => {
                        e.stopPropagation();
                        openProductKioskConfiguration(product);
                      }}
                      aria-label={tr('control.products.configuration', 'Configuration')}
                      title={tr('control.products.configuration', 'Configuration')}
                    >
                      <span>{tr('control.products.configuration', 'Configuration')}</span>
                    </button>
                  </span>
