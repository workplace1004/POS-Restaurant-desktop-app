  ...
  return (
    <div className="relative min-h-[650px] rounded-xl border border-pos-border bg-pos-panel/30 p-4 pb-[60px] flex flex-col">
          ...
          >
            {categories.map((cat) => (
              <button
                ...
                type="button"
                className={`px-4 py-2 text-sm font-medium whitespace-nowrap shrink-0 transition-colors border-b-2 ${
      ...
      >
        {!selectedCategoryId ? (
        ...
        ) : productsLoading ? null : filteredProducts.length === 0 ? (
          ...
          <ul className="w-full flex flex-col">
            {filteredProducts.map((product) => (
              <li
                key={product.id}
                className={`flex items-center w-full justify-between px-4 py-2 border-y border-pos-panel text-pos-text text-sm cursor-pointer ${
                  selectedProductId === product.id ? 'bg-pos-panel/70' : 'bg-pos-bg'
                }`}
                onClick={(e) => {
                  if (!e.target.closest('button')) setSelectedProductId(product.id);
                }}
              >
                ...
                <span className="flex-shrink-0 min-w-[30%] text-center text-sm text-white">
                  <button
                    type="button"
                    className={`px-2 py-1 rounded text-sm text-white active:text-pos-text active:bg-green-500 ${
                      productHasSubproductsById[product.id] ? 'font-medium' : ''
                    }`}
                    onClick={(e) => {
                      e.stopPropagation();
