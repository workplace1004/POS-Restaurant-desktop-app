  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative bg-pos-bg rounded-xl shadow-2xl max-w-[90%] w-full justify-center items-center mx-4 overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        <button type="button" className="absolute top-4 right-4 z-10 p-2 rounded text-black active:text-white active:bg-rose-500" onClick={closeCategoryModal} aria-label="Close">
          <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
        <div className="p-6 flex flex-col space-y-6 w-full justify-center text-lg items-center pt-20">
          <div className="w-full flex flex-col items-center gap-5">
            <div className="flex gap-2 w-full items-center pl-[30%]">
              <label className="block min-w-[150px] max-w-[150px] font-medium text-black">{tr('name', 'Name')} : </label>
              <input
                type="text"
                value={categoryName}
                onChange={(e) => setCategoryName(e.target.value)}
                className="px-4 w-[300px] h-[40px] py-3 border border-gray-300 text-black rounded-lg"
                onFocus={() => setCategoryActiveField('name')}
                onClick={() => setCategoryActiveField('name')}
              />
            </div>
            <div className="flex gap-2 w-full items-center pl-[30%]">
              <label className="block min-w-[150px] max-w-[150px] font-medium text-black">{tr('control.inWebshop', 'In webshop')} : </label>
            ...
            <div className="flex gap-2 w-full items-center pl-[30%]">
              <label className="block min-w-[150px] max-w-[150px] font-medium text-black">{tr('control.displayOnThisCashRegister', 'Display on this cash register')} : </label>
              <div className="w-[200px] flex items-center justify-start">
                <input
                  type="checkbox"
            ...
            <div className="flex gap-2 w-full items-center pl-[30%]">
              <label className="block min-w-[150px] max-w-[150px] font-medium text-black">{tr('nextCourse', 'Next course')} : </label>
              <input
                type="text"
                value={categoryNextCourse}
                onChange={(e) => setCategoryNextCourse(e.target.value)}
                className="px-4 w-[300px] h-[40px] py-3 border border-gray-300 rounded-lg text-black"
